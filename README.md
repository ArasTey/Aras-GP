...
## 🔁 اجرای دائمی و مدیریت (اسکریپت `aras`)

یک منوی واحد برای همه‌ی کارها — نصب، آپدیت، روشن/خاموش، تغییر پورت و رمز، و…
درست مثل `x-ui` ولی چون با پایتون نوشته شده روی **لینوکس، مک و ویندوز** یکسان کار
می‌کند. برای اجرا:

```bash
./aras.sh            # لینوکس / مک — منوی مدیریت باز می‌شود
```
```bat
aras.bat            :: ویندوز
```
...
```
╔────────────────────────────────────────────────────╗
│  Aras-GP  —  مدیریت پنل   v2.1.5                   │
│  0. خروج                                           │
│────────────────────────────────────────────────────│
│  1. نصب   2. به‌روزرسانی   3. نسخه‌ی مشخص   4. حذف     │
│────────────────────────────────────────────────────│
│  5. روشن   6. خاموش   7. ری‌استارت   8. وضعیت   9. لاگ │
│────────────────────────────────────────────────────│
│  10. تغییر پورت   11. تغییر رمز                      │
│  12. دسترسی از شبکه (LAN)   13. مشاهده‌ی تنظیمات      │
│────────────────────────────────────────────────────│
│  14. پشتیبان‌گیری   15. خروجی به مسیر دلخواه          │
│  16. بازگردانی از پشتیبان   17. بازنشانی به اولیه     │
│────────────────────────────────────────────────────│
│  18. اتواستارت روشن   19. اتواستارت خاموش            │
│  20. باز کردن پنل در مرورگر                          │
╚────────────────────────────────────────────────────╝
  وضعیت پنل   : در حال اجرا   http://127.0.0.1:8600
  اتواستارت   : بله
```

هنگام نصب می‌پرسد که دستور سراسری `aras` ساخته شود؛ اگر بله، از آن به بعد فقط
کافی است هرجا `aras` بزنید (مثل `x-ui`). هر گزینه هم به‌صورت مستقیم قابل اجراست:

```bash
./aras.sh start | stop | restart | status | install | version
```
...
> اسکریپت `aras` خودش وابستگی ندارد و بدون venv هم اجرا می‌شود؛ برای همین حتی
> روی یک ماشین تازه که هنوز چیزی نصب نشده، گزینه‌ی «نصب» کار می‌کند.
