...
# How often the account sweeper checks for users that ran out of quota or
# whose subscription expired while they were connected.
ACCOUNT_SWEEP_INTERVAL = 5.0
...
class ProxyServer:
    ...
    def __init__(self, config: dict):
        ...
        # Per-connection accounts. Dormant unless config["proxy_auth"]["enabled"]
        # is set, so a personal single-user setup keeps its old behaviour.
        self.accounts = AccountManager(config)
        # True once the SOCKS5 listener has actually bound. A busy port is
        # logged and tolerated below, so callers need a way to tell the
        # difference between "configured" and "really listening".
    ...
    async def start(self):
        ...
        self.socks_active = socks_srv is not None

        if self.accounts.required and self._sweeper is None:
            self._sweeper = asyncio.create_task(self._account_sweeper())
    ...
    async def _account_sweeper(self):
        """Tear down connections of users who ran out of quota or expired."""
        while True:
            try:
                await asyncio.sleep(ACCOUNT_SWEEP_INTERVAL)
                self.accounts.sweep()
            except asyncio.CancelledError:
                break
            except Exception as exc:  # never let the sweeper kill the server
                log.debug("Account sweep error: %s", exc)

    @staticmethod
    def _extract_proxy_auth(header_block: bytes) -> str | None:
        """Pull the Proxy-Authorization credential out of a raw header block."""
        for raw_line in header_block.split(b"\r\n")[1:]:
            name, sep, value = raw_line.partition(b":")
            if sep and name.strip().lower() == b"proxy-authorization":
                return value.strip().decode("latin-1", errors="replace")
        return None

    @staticmethod
    def _strip_proxy_auth(header_block: bytes) -> bytes:
        """Remove hop-by-hop proxy credentials before the request is relayed."""
        lines = header_block.split(b"\r\n")
        kept = [lines[0]]
        for raw_line in lines[1:]:
            name, sep, _ = raw_line.partition(b":")
            if sep and name.strip().lower() in (b"proxy-authorization",
                                                b"proxy-connection"):
                continue
    ...
    def _auth_challenge(self) -> bytes:
        realm = self.accounts.realm.replace('"', "")
        return (
            f'HTTP/1.1 407 Proxy Authentication Required\r\n'
            f'Proxy-Authenticate: Basic realm="{realm}", charset="UTF-8"\r\n'
    ...
    async def _on_client(self, reader: asyncio.StreamReader, writer: asyncio.StreamWriter):
        ...
        try:
            ...
            # ── per-user authentication (RFC 7235 proxy auth) ─────────
            if self.accounts.required:
                account = self.accounts.authenticate_basic(
                    self._extract_proxy_auth(header_block)
                )
                if account is None:
                    log.info("HTTP proxy auth rejected: %s", addr)
                    writer.write(self._auth_challenge())
                    await writer.drain()
                    return
                account.attach(task)
                # Everything from here on is billed to this account.
                reader = CountedReader(reader, account)
                writer = CountedWriter(writer, account)

            # Proxy-Authorization and Proxy-Connection are hop-by-hop: they are
            # addressed to *this* proxy and must never travel upstream, whether
            # or not we required credentials.
    ...
    async def _socks_userpass_auth(self, reader, writer, addr):
        """RFC 1929 username/password sub-negotiation. Returns the account."""
        head = await asyncio.wait_for(reader.readexactly(2), timeout=15)
        if head[0] != 0x01:  # sub-negotiation version
            writer.write(b"\x01\x01")
            await writer.drain()
            return None
        username = (await asyncio.wait_for(
            reader.readexactly(head[1]), timeout=10)).decode(errors="replace")
        plen = (await asyncio.wait_for(reader.readexactly(1), timeout=10))[0]
        password = (await asyncio.wait_for(
            reader.readexactly(plen), timeout=10)).decode(errors="replace")

        account = self.accounts.authenticate(username, password)
        if account is None:
            log.info("SOCKS5 auth rejected for %r: %s", username, addr)
    ...
    async def _on_socks_client(self, reader: asyncio.StreamReader,
        ...
        try:
            ...
            if self.accounts.required:
                # 0x02 = username/password (RFC 1929). No anonymous fallback.
                ...
                await writer.drain()
                account = await self._socks_userpass_auth(reader, writer, addr)
                if account is None:
                    return
                account.attach(task)
                reader = CountedReader(reader, account)
